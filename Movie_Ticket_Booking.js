...
function chooseuser()
{
console.log("\nSelect User: 1. Admin 2. User 3. Exit"); // User Selection
...
function addMovie(){
    ...
    connection.query('INSERT INTO movie SET ?', movie, function(err, result) {
        ...
        var data = [['Movie Name', 'Movie Time', 'Movie Date', 'Movie Seats', 'Movie Price']];
        data.push([ movie.movieName, movie.movieTime, movie.movieDate, movie.movieSeats, movie.moviePrice]);
...
function displayMovies()
{
    ...
    connection.query('SELECT * FROM movie', function(err, result) {
        if (err) throw err;
        var table = require('table');
        var data = [['Movie Number', 'Movie Name', 'Movie Time', 'Movie Date', 'Movie Seats', 'Movie Price']];
        for (var i = 0; i < result.length; i++) {
            data.push([result[i].movieNumber, result[i].movieName, result[i].movieTime, result[i].movieDate, result[i].movieSeats, result[i].moviePrice]);
        }
        var output = table.table(data);
        console.log(output);
        var loop = require('readline-sync').question("\nDo you want to continue (Y/N)?");
...
function editTicket()
{
    ...
    connection.query('UPDATE movie SET ? WHERE movieNumber = ?', [movie, movienumber], function(err, result) {
        if (err) throw err;
...
function deleteMovies(){
    console.log("\n\n********************** All Scheduled Shows *********************");
        connection.query('SELECT * FROM movie', function(err, result) {
            ...
            //delete show
    console.log("\n\n********************** Delete Show *********************");
    var movienumber = require('readline-sync').question("Select Movie Number: ");
    connection.query('DELETE FROM movie WHERE movieNumber = ?', movienumber, function(err, result) {
        if (err) throw err;
...
function displayTable(){
    ...
    var data = [
        ['Customer Name', 'Customer Email', 'Customer Phone', 'Movie Name', 'Movie Time', 'Movie Date', 'Movie Seats', 'Total Price']]
    connection.query('SELECT * FROM userinfo', function(err, result) {
        if (err) throw err;
        for (var i = 0; i < result.length; i++) {
            data.push([result[i].customerName, result[i].customerEmail, result[i].customerPhone, result[i].movieName, result[i].movieTime, result[i].movieDate, result[i].customerSeats, result[i].totalPrice]);
        }
...
function bookTicket(){
    ...
    var movienumber = require('readline-sync').question("Select Movie Number: ");
    var userSeats = require('readline-sync').question("Enter Number of Seats: ");
    
    //fetch movie details from movie table using movie number and store in variable
    connection.query('SELECT * FROM movie WHERE movieNumber = ?', movienumber, function(err, result) {
        if (err) throw err;
    ...
    connection.query('INSERT INTO userinfo SET ?', userdetails, function(err, result) {
        if (err) throw err;
        ...
        var data = [
            ['Customer Name', 'Customer Email', 'Customer Phone', 'Movie Name', 'Movie Time', 'Movie Date', 'Movie Seats', 'Total Price']]
        connection.query('SELECT * FROM userinfo where customerPhone = ?',userPhone, function(err, result) {
            if (err) throw err;
            for (var i = 0; i < result.length; i++) {
                data.push([result[i].customerName, result[i].customerEmail, result[i].customerPhone, result[i].movieName, result[i].movieTime, result[i].movieDate, result[i].customerSeats, result[i].totalPrice]);
            }
